...
import { isSafeUrl } from "core/utils/security"
        ...
        if (spec && specSelectors.isOAS3() && path[0] === "components" && path[1] === "securitySchemes") {
          ...
          await Promise.all(Object.values(spec)
            ...
            .map(async (oidcScheme) => {
              if (!isSafeUrl(oidcScheme.openIdConnectUrl)) {
                console.error(`Blocked unsafe OIDC discovery URL: ${oidcScheme.openIdConnectUrl}`)
                return
              }
              const req = {
