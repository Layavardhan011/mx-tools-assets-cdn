    ...
    if (
      ...
      if ("Blob" in window) {
        ...
        if(win.navigator && win.navigator.msSaveOrOpenBlob) {
            bodyEl = <div><MsSaveOrOpenBlobLink href={ href } blob={blob} download={download} /></div>
        } else {
...
class MsSaveOrOpenBlobLink extends React.Component {
  handleClick = () => {
    const { blob, download } = this.props
    win.navigator.msSaveOrOpenBlob(blob, download)
  }
  render() {
    return <a href={this.props.href} onClick={this.handleClick}>{ "Download file" }</a>
  }
}

MsSaveOrOpenBlobLink.propTypes = {
